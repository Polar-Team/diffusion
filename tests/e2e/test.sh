...
while [[ $# -gt 0 ]]; do
  ...
  --help)
    ...
    echo "  --provider <n>   Vagrant provider (virtualbox, vmware_desktop, hyperv, libvirt)"
    ...
    echo "  $0 --provider virtualbox              # Force VirtualBox provider"
    echo "  $0 --provider hyperv                  # Force Hyper-V provider"
...
if [ -n "$PROVIDER" ]; then
  ...
  echo -e "${CYAN}Provider: (Vagrant default)${NC}"
fi
echo ""
