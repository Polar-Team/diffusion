...
`
	...
	if len(loadedConfig.DependencyConfig.Collections) != len(expectedCollections) {
		t.Errorf("Expected %d collections, got %d", len(expectedCollections), len(loadedConfig.DependencyConfig.Collections))
	}

	for _, col := range loadedConfig.DependencyConfig.Collections {
	...
	if len(loadedConfig.DependencyConfig.Roles) != len(expectedRoles) {
		t.Errorf("Expected %d roles, got %d", len(expectedRoles), len(loadedConfig.DependencyConfig.Roles))
	}

	for _, role := range loadedConfig.DependencyConfig.Roles {
	...
	t.Logf("✅ Successfully scanned and imported %d collections and %d roles", len(loadedConfig.DependencyConfig.Collections), len(loadedConfig.DependencyConfig.Roles))
}
...
func TestDepsInitWithNoExistingFiles(t *testing.T) {
	...
	if loadedConfig.DependencyConfig == nil {
		t.Error("DependencyConfig should not be nil")
	}

	if len(loadedConfig.DependencyConfig.Collections) != 0 {
		t.Errorf("Expected 0 collections, got %d", len(loadedConfig.DependencyConfig.Collections))
	}

	if len(loadedConfig.DependencyConfig.Roles) != 0 {
		t.Errorf("Expected 0 roles, got %d", len(loadedConfig.DependencyConfig.Roles))
	}
...
`, scenario.roleName, scenario.roleName)
	...
	if len(loadedConfig.DependencyConfig.Roles) != len(expectedRoleNames) {
		t.Errorf("Expected %d roles, got %d", len(expectedRoleNames), len(loadedConfig.DependencyConfig.Roles))
	}
