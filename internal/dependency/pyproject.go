...
func GeneratePyProjectContent(collections []config.CollectionRequirement, toolVersions map[string]string, pythonVersion *config.PythonVersion) (string, error) {
...
func GeneratePyProjectTOML(collections []config.CollectionRequirement, toolVersions map[string]string, pythonVersion *config.PythonVersion, outputPath string) error {
...
func formatPythonRequirement(pythonVersion *config.PythonVersion) string {
...
func SyncPyProjectWithLock(lockFile *LockFile, containerProjectPath string) error {
	...
	collections := make([]config.CollectionRequirement, 0)
