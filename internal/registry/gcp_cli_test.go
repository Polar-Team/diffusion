...
func TestGcpCliInit(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantErr:        false, // Will fail without actual gcloud credentials
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			os.Unsetenv(config.EnvGCPProjectID)
...
func TestGcpCliInitSuccessfulAuth(t *testing.T) {
	...
	if err == nil {
		...
		if token == "" {
			t.Errorf("%s environment variable should be set after successful GcpCliInit", config.EnvToken)
		}
