...
func TestAddCollectionOnlyModifiesToml(t *testing.T) {
	...
	if err := config.SaveConfig(initialConfig); err != nil {
	...
	newCollection := config.CollectionRequirement{Name: "community.docker", Version: ">=5.0.0"}
	cfg.DependencyConfig.Collections = append(cfg.DependencyConfig.Collections, newCollection)
	...
	updatedConfig, err := config.LoadConfig()
	if err != nil {
...
func TestDepsSyncUpdatesMetaWithoutVersions(t *testing.T) {
	...
	if err := dependency.SaveLockFile(lockFile); err != nil {
		t.Fatalf("Failed to create lock file: %v", err)
	}

	// Load role config (creates empty files if they don't exist)
	meta, req, err := role.LoadRoleConfig("default")
	if err != nil {
	...
	// Save files
	if err := role.SaveRequirementFile(req, "default"); err != nil {
	...
	loadedMeta, loadedReq, err := role.LoadRoleConfig("default")
	if err != nil {
		t.Fatalf("Failed to load role config: %v", err)
	}
