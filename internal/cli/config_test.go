...
`
	...
	if cfg.ContainerRegistry.RegistryServer != "cr.example.com" {
		t.Errorf("unexpected registry server: got %q, want %q",
			cfg.ContainerRegistry.RegistryServer, "cr.example.com")
	}

	if cfg.ContainerRegistry.RegistryProvider != "YC" {
		t.Errorf("unexpected registry provider: got %q, want %q",
			cfg.ContainerRegistry.RegistryProvider, "YC")
	}

	if cfg.HashicorpVault == nil {
...
func TestSaveConfig(t *testing.T) {
	...
	if loadedCfg.ContainerRegistry.RegistryServer != cfg.ContainerRegistry.RegistryServer {
		t.Errorf("registry server mismatch: got %q, want %q",
			loadedCfg.ContainerRegistry.RegistryServer,
