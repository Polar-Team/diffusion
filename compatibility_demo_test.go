...
func TestCompatibilityDemo(t *testing.T) {
	...
	adjusted, warnings := AdjustToolVersionsForPython(toolVersions, "3.9")
	...
	adjusted2, warnings2 := AdjustToolVersionsForPython(toolVersions2, "3.10")
	...
	for _, pyVer := range []string{"3.9", "3.10", "3.13"} {
		fmt.Printf("\nPython %s:\n", pyVer)
		recommended := GetRecommendedVersions(pyVer)
