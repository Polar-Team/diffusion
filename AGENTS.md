...
## On-Demand Workflows (manual)

These are available as slash commands (defined in `.opencode/command/`). The command file is the full prompt — invoke it directly:

- `/sync-docs-website` — Scan all Diffusion ecosystem repos for changes and update the docs website (`docs/index.html`, `docs/changelog.md`).
- `/update-mcp-docs` — Review source changes and update the MCP server data, docs, troubleshooting cases, and helpers under `mcp/diffusion_mcp`.
