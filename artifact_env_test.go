...
func TestIndexedEnvironmentVariables(t *testing.T) {
	...
	defer func() {
		for i := 1; i <= MaxArtifactSources; i++ {
			os.Unsetenv(fmt.Sprintf("%s%d", EnvGitUserPrefix, i))
			os.Unsetenv(fmt.Sprintf("%s%d", EnvGitPassPrefix, i))
			os.Unsetenv(fmt.Sprintf("%s%d", EnvGitURLPrefix, i))
		}
	}()
	...
	for _, ts := range testSources {
		os.Setenv(fmt.Sprintf("%s%d", EnvGitUserPrefix, ts.index), ts.username)
		os.Setenv(fmt.Sprintf("%s%d", EnvGitPassPrefix, ts.index), ts.password)
		os.Setenv(fmt.Sprintf("%s%d", EnvGitURLPrefix, ts.index), ts.url)
	}

	// Verify all variables are set correctly
	for _, ts := range testSources {
		gotUser := os.Getenv(fmt.Sprintf("%s%d", EnvGitUserPrefix, ts.index))
		if gotUser != ts.username {
			t.Errorf("GIT_USER_%d: got %q, want %q", ts.index, gotUser, ts.username)
		}

		gotPass := os.Getenv(fmt.Sprintf("%s%d", EnvGitPassPrefix, ts.index))
		if gotPass != ts.password {
			t.Errorf("GIT_PASSWORD_%d: got %q, want %q", ts.index, gotPass, ts.password)
		}

		gotURL := os.Getenv(fmt.Sprintf("%s%d", EnvGitURLPrefix, ts.index))
		if gotURL != ts.url {
...
func TestMaxArtifactSources(t *testing.T) {
	if MaxArtifactSources < 1 {
		t.Error("MaxArtifactSources should be at least 1")
	}

	if MaxArtifactSources > 100 {
		t.Error("MaxArtifactSources seems unreasonably high")
	}
	...
	for i := 1; i <= MaxArtifactSources; i++ {
		varName := fmt.Sprintf("%s%d", EnvGitUserPrefix, i)
...
func TestEnvironmentVariableConstants(t *testing.T) {
	tests := []struct {
		...
		{"EnvGitUserPrefix", EnvGitUserPrefix, "GIT_USER_"},
		{"EnvGitPassPrefix", EnvGitPassPrefix, "GIT_PASSWORD_"},
		{"EnvGitURLPrefix", EnvGitURLPrefix, "GIT_URL_"},
	}
...
func TestGetArtifactCredentialsIntegration(t *testing.T) {
	...
	// Get credentials
	retrieved, err := GetArtifactCredentials(source, nil)
	if err != nil {
...
func TestMultipleArtifactSourcesSimulation(t *testing.T) {
	...
	for _, s := range sources {
		...
		defer DeleteArtifactCredentials(s.name)
	}
	...
	for i, s := range sources {
		...
		creds, err := GetArtifactCredentials(source, nil)
		if err != nil {
			t.Fatalf("failed to get credentials for %s: %v", s.name, err)
		}

		// Set environment variables
		os.Setenv(fmt.Sprintf("%s%d", EnvGitUserPrefix, index), creds.Username)
		os.Setenv(fmt.Sprintf("%s%d", EnvGitPassPrefix, index), creds.Token)
		os.Setenv(fmt.Sprintf("%s%d", EnvGitURLPrefix, index), creds.URL)
	}

	// Verify all environment variables are set correctly
	for i, s := range sources {
		index := i + 1
		gotUser := os.Getenv(fmt.Sprintf("%s%d", EnvGitUserPrefix, index))
		if gotUser != s.username {
			t.Errorf("GIT_USER_%d: got %q, want %q", index, gotUser, s.username)
		}

		gotPass := os.Getenv(fmt.Sprintf("%s%d", EnvGitPassPrefix, index))
		if gotPass != s.token {
			t.Errorf("GIT_PASSWORD_%d: got %q, want %q", index, gotPass, s.token)
		}

		gotURL := os.Getenv(fmt.Sprintf("%s%d", EnvGitURLPrefix, index))
		if gotURL != s.url {
