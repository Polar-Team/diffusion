...
func TestComputeDependencyHash(t *testing.T) {
	...
	collections2 := append(collections, CollectionRequirement{Name: "kubernetes.core", Version: ">=2.4.0"})
...
func TestFormatPythonRequirement(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := formatPythonRequirement(tt.version)
			if got != tt.want {
				t.Errorf("formatPythonRequirement() = %q, want %q", got, tt.want)
			}
		})
	}
}
...
func TestFormatDependency(t *testing.T) {
	tests := []struct {
		...
		{"no version", "ansible", "", "ansible"},
		{"latest", "ansible", "latest", "ansible"},
		{"with >=", "ansible", ">=10.0.0", "ansible>=10.0.0"},
		{"with ==", "ansible", "==10.0.0", "ansible==10.0.0"},
		{"plain version", "ansible", "10.0.0", "ansible>=10.0.0"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := formatDependency(tt.pkg, tt.version)
			if got != tt.want {
				t.Errorf("formatDependency(%q, %q) = %q, want %q", tt.pkg, tt.version, got, tt.want)
			}
		})
	}
}
...
func TestGetCollectionPythonDependencies(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.collection, func(t *testing.T) {
			deps := getCollectionPythonDependencies(tt.collection)
			if len(deps) != tt.wantCount {
				t.Errorf("getCollectionPythonDependencies(%q) returned %d deps, want %d", tt.collection, len(deps), tt.wantCount)
			}
		})
	}
}
