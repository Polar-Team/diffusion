...
func TestConfigWithArtifactSources(t *testing.T) {
	...
	if len(config.ArtifactSources) != 2 {
		t.Errorf("expected 2 artifact sources, got %d", len(config.ArtifactSources))
	}

	// Verify first source (local)
	if config.ArtifactSources[0].UseVault {
		t.Error("first source should use local storage")
	}

	// Verify second source (vault)
	if !config.ArtifactSources[1].UseVault {
		t.Error("second source should use vault storage")
	}

	if config.ArtifactSources[1].VaultPath == "" {
...
func TestEmptyArtifactSources(t *testing.T) {
	config := &Config{
		...
		ArtifactSources: []ArtifactSource{}, // Empty
		YamlLintConfig: &YamlLint{
			Extends: "default",
			Ignore:  []string{},
			Rules:   &YamlLintRules{},
	...
	if len(config.ArtifactSources) != 0 {
		t.Errorf("expected 0 artifact sources, got %d", len(config.ArtifactSources))
	}
